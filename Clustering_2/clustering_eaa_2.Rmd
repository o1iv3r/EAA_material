...
We'll filter out NAs here and focus only on the clustering part. In practice, you could use mice  to complete the data, but it may be a good starting point to first consider only available data. Perhaps on of the missing features turns out to be irrelevant.
...
One can use the clustMixType as in the first part of the course. Alternatively,one can use partitioning around medoids, where one computes the distance matrix using Gower's distance a-priori. However, this approach ist tather slow (and memory intensive) and works only for smaller data sets
...
Similarly as with k-means, data with mixed data types can per se be used with DBSCAN if the pairwise distances are computed a-priori.
...
Multivariate diagonal Gaussian Mixture Models + Dirichlet Distribution(?)

Let us write a multidimensional Gaussian mixture model in the fromGaussian_s*withs*,the different ways to parameterize the standard-deviation parameters of a Gaussian mixture: sjk means  that  we  have  one  standard-deviation  parameter  for  each  variable  in  eachcomponen

pjk: probably means that there is one probability for each level of a categorical variable and each component

All models are conditionally independent (given the parameters), thus the density of each component (i.e., cluster) is the product of all univariate densities.

For the purely numerical data we were estimating covariance matrices without the restriction of them being diagonal. This is the trade of in this situation.
...
# pred_clustermix <- clusterPredict(ldata,clustermix_res) # for new data
...
Higher value for GMM

```{r}
gmm_db <- index.DB(dat_num_no_missings_scaled, pred_gmm_res$cluster_labels)
gmm_db$DB
```
...
Note: Before applying this function to DBSCAN one hast to take out the rows belonging to the outlier cluster 0! Thus the result is not comparable to the one for the other algorithms
...
# todo use predictGMM as prediction function
...
## Interpretation using supervised ML

A simple supervised machine learning algorithm can be easier to interpret than a complex unsupervised algorithm.

OneR will pick one single variable and then tries to find rules that associate its values with the target. 
...
```{r}
library(OneR)
dat_with_label <- dat_num_no_missings_scaled_df
dat_with_label$cluster <- factor(pred_gmm_res$cluster_labels)
gmm_formula <- as.formula(paste0("cluster ~ ",paste(colnames(dat_num_no_missings_scaled_df),collapse = " + ")))
gmm_formula
```
...
```{r}
gmm_proxy_model <- OneR(formula=gmm_formula, data=dat_with_label)
gmm_proxy_model
```

2/3rd of our ships can be correctly classified using ShipLength only! However, note that this is not a valid evaluation of our one-rule algorithm, since we are using the training set. The performance can be much worse on a new list of shops.

Nevertheless we observe that the length is a quite relevant feature, and while cluster 4 and 5 contain mostly smaller ships, 1 and 6 contain rather large ships.

This is not limited to OneR, one could use decision tree or a GLM as well!
...
DBSCAN is the only algorithm among the ones we are focusing here that has a build-in concept of outlier points.
...
Outlier: probability of belonging to assigned cluster is below 50% (this can happen since we have more than 2 clusters)
