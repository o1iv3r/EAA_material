...
Based on the result we select 5 clusters (and our analysis in the first part of this course)
...
We only have to specify the minPts parameter. Note that not all points are clustered.

```{r}
library(dbscan)
dbscan_res <- hdbscan(dat_num_no_missings_scaled, minPts = 7)
dbscan_res
```

The eps value we can set post-hoc to determine the number of clusters?

```{r}
plot(dbscan_res, show_flat=T)
```


todo

### Probabilistc clustering
