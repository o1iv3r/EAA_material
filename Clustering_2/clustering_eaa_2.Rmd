...
3. Given a fitted *GMM* computed on the $m$ variables $(x_{f_1},\ldots,x_{f_m})$, perform an add and remove step and accept or reject based on whether BIC improves or not:
* Add step to determine the best larger model: Fit a GMM for all pairs $(x_{f_1},\ldots,x_{f_m},x_j)$ for all $j\notin \{f_1,\ldots,f_m\}$ and keep the variable $f_{m+1}:=j$ so that the new model *GMM_Add* hat the highest BIC among all alternatives that include variables $\{f_1,\ldots,f_m\}$.
...
* Decision step: Among *GMM*, *GMM_Add* and *GMM_Remove*, return the model *GMM_new* with the highest BIC and repeat step 3.
...
```{r}
load("Clustering_2\\FeatureSelectionGMM.Rdata")
```
...
```
Thus the final clustering is only based on two variables:

```{r, warning=FALSE}
clustvarsel_res_forward$subset
```

The procuedure removes four variables before it stops (note that step 4 seems redundant).
