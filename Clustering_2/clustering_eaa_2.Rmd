...
$ M_{i,j} = (\sum_{k=1}^n |c_{i,k}-c_{j,k}|^p)^{1/p} $. For $p=2$ this is simply the Euclidean distance between the centroids.

The separation of cluster $i$ from all other cluster can then be computed by $D_i = max_{j\neq i}\frac{S_i+S_j}{M_{i,j}}$, that is, for the "worst case" pairing with cluster $i$, the ratio between the sum of within cluster dispersions and the between cluster distance.

The DB index is then defined as the average over all clusters
$DB = \frac{1}{k}\sum_{i=1}^k D_i$. Thus one computes, for the "worst case" pair of clusters, the 

Consequently, a lower DB implies a better separation: Then, even all clusters are rather far apart and tightly centered around their mean.
...
Before we can calculate the feature importance of oru Gaussian-Mixture-Model, we have to define a prediction function the maps the cluster object and a data table to a vector with integers, the cluster assignments per row of the data table.

```{r}
predictGMM_labels <- function(gmm,dat){
  return (predictGMM(gmm,dat)$cluster_labels)
}
```

ToDo Fix Bug in FeatureImpCluster first
