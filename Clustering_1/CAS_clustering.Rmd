...
(we'll restrict the pre-processing to a bar minimum in this course)
...
We see that 6 clusters provide a quite stable assignment, so this so will work with 6 clusters from now on.
...
```{r , fig.height = 12, fig.width = 15, warnings=FALSE}
...
> YES OR NO: In order to increase the importance of the categorical variables, we have to increase the value of lambda?
